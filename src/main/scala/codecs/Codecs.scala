...
trait DecoderInstances:
  ...
  given stringDecoder: Decoder[String] = Decoder.fromPartialFunction { case Json.Str(n) => n }

  /** A decoder for `Boolean` values */
  // TODO Define a given instance of type `Decoder[Boolean]`
  given boolDecoder: Decoder[Boolean] = Decoder.fromPartialFunction { case Json.Bool(n) => n }
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] = {
      val decodedItem = decoder.decode(item)
      if (decodedItem.isDefined)
        Some(decodedItem.get +: previouslyDecodedItems)
      else None
    }
    ...
    Decoder.fromFunction {
      case Json.Arr(array) => decodeAllItems(array)
      case _ => None
    }
